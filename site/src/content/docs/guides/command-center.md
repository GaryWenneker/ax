...
| **Policy** | View-first rule/skill editors with **Scope** (company → private); Rules/Skills list **label autocomplete** (tag chips, multi-label AND filter, click table tags to toggle); tags required on save; layer filters; **Sync** (pack export/import + `policySync` hooks); **Review** queue for staged imports |
