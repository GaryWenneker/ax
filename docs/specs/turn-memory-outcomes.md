...
6. **Review fixes (no behavior row changes).** Preflight and `ax_history` select only candidate turns in SQL (recalled ids, or `json_each(files)` matching a path or `/<suffix>`) instead of loading every turn body. `git log` runs through `tokio::process`, and the path canonicalization in preflight uses `tokio::fs`. Pruning deletes in one transaction. A prompt that quotes the text `"\n\nOutcome: "` can no longer pose as the outcome: in the part before the real outcome it is stored as `"\n\nOutcome - "`. New tests cover the unindexed path suffix, the candidate limit and the quoted marker.
7. **git ignores a far-future `--since`.** git rejects `--since` timestamps from 2100-01-01 on and falls back to about "now", so `ax_history --since 2999-01-01` listed commits made in the last few seconds. `ax_history` now also filters commits on `since` itself; git's `--since` only shortens the walk. Found by layer 7 once its `HOME` was isolated (next point), and covered by `h3_since_in_the_future_drops_a_commit_made_just_now`.
8. **The gauntlet's real-execution layer ran `ax init` with the real `HOME`.** `ax init` writes the user's global Cursor and Claude hooks with the path of the running binary, so both gauntlets had pointed the user's hooks at a `/tmp` binary. Layer 7 now runs with `HOME` set to a temp dir and fails if `~/.cursor/hooks.json` or `~/.claude/settings.json` changed. The user's hook files were repaired to `~/.local/bin/ax`.
