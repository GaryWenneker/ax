...
**source:** working tree after Local-newer compare color (red)
...
== web helpers ==
  11 passed (policyPackage.test.ts)
== wiring ==
== negative control ==
== tsc ==
== manual mutation ==
...
| B9 colors | `compareStatusClass('changed','local')` → `--local` (`--danger`); package newer → `--changed` (`--warn`); identical → `--identical` (`--ok`) |
