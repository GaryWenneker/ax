...
pub async fn run(path: Option<String>, quiet: bool, watch: bool) -> Result<(), String> {
    ...
    if watch {
        if !quiet {
            println!("{} Watching for file changes (Ctrl+C to stop)...", glyphs::ok());
        }
        tokio::select! {
            ...
            _ = tokio::signal::ctrl_c() => {
                ax.unwatch().await;
                if !quiet {
                    println!("{} Stopped watching", glyphs::ok());
                }
                Ok(())
            }
        }
        ...
        if !quiet {
            println!("Synced {} files in {}ms", result.files_indexed, result.duration_ms);
        }
        Ok(())
    }
}
