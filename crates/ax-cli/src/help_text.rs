...
Formula: (unique files × avg file tokens) − graph response tokens. Each ax_explore / graph
MCP call logs response size and a counterfactual file-read estimate in ~/.ax/usage.db.
